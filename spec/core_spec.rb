...
module Anemone
  describe Core do
    ...
    shared_examples_for "crawl" do
      ...
      it "should be able to limit links to crawl based on a RegEx" do
        pages = []
        pages << FakePage.new('0', :links => ['1', '2', 'foo-3'])
        pages << FakePage.new('1')
        pages << FakePage.new('2')
        pages << FakePage.new('foo-3')

        core = Anemone.crawl(pages[0].url, @opts) do |a|
          a.only_crawl_links_like /0/, /1/, /foo-*/
        end

        core.should have(3).pages
        core.pages.keys.should include(pages[1].url)
        core.pages.keys.should_not include(pages[2].url)
        core.pages.keys.should include(pages[3].url)
      end
